...
export const CategoryGrid = ({
  ...
  return <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-12 animate-fade-in-up">
        <h2 className="text-4xl font-bold text-shimmer mb-4">Categories</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {categories.map((category, index) => <Card key={category.id} className={`category-card group cursor-pointer bg-card border-border/50 
                       hover:border-primary/30 transition-all duration-500 overflow-hidden
                       hover-lift hover-glow animate-fade-in-up`} style={{
        animationDelay: `${index * 0.1}s`
      }} onMouseEnter={() => setHoveredCategory(category.id)} onMouseLeave={() => setHoveredCategory(null)} onClick={() => handleCategoryClick(category)}>
            <div className="relative aspect-square overflow-hidden">
              <img src={category.image} alt={category.title} className="w-full h-full object-cover transition-transform duration-700 
              ...
              <div className={`absolute top-4 right-4 w-3 h-3 rounded-full bg-primary
                             transform transition-all duration-300 ${hoveredCategory === category.id ? 'scale-100 animate-pulse-glow' : 'scale-0'}`} />
            </div>
          </Card>)}
      </div>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        {Array.from({
        length: 20
      }).map((_, i) => <div key={i} className="absolute w-1 h-1 bg-primary/20 rounded-full animate-float" style={{
